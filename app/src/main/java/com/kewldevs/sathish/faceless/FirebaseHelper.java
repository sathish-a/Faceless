...
public class FirebaseHelper {
     ...
     * |                         |                          |
     * ActiveLists                Users                    BucketLists
     * |                        |                          |
     * UID                      UID                        UID
     * |                        |                          |
     * GEO_HASHES           Profile_Information      _________|__________
     * |                  |
     * items             viewed
     * |
     * PUSH_ID
     ...
     * Items_information
     * <p>
     * Active list is updated with user location only if that respective user has one or more items in the Bucket Lists
     **/

    public static void setmUser(FirebaseUser User) {
        //Initialize every nodes
        FirebaseHelper.mUser = User;
        mRoot = FirebaseDatabase.getInstance().getReference(); //gives the root node of db
        mUserReference = mRoot.child("Users"); // gives the users node
        mActiveReference = mRoot.child("ActiveList");  // gives the active lists node
        mBucketListRefernce = mRoot.child("BucketLists"); // gives the bucket lists node
        mGeoActiveReference = new GeoFire(mActiveReference); // updates the active list with geo location and hashes
        mStorageRoot = FirebaseStorage.getInstance().getReference(); // gives the root dir of storage
        myProfile = null; //contains profile info
        isInfoPresent = false; // has the value whether profile is fully updated
        setUID(User.getUid());
    }

    public static void downloadUserProfileDatas() {
        //Downloads user profile information
    ...
    public static void checkForEmptyBucket(final SwipeRefreshLayout swipeRefreshLayout, final Context context) {
            //If user has one or more items then the activelists is updated i.e Online mode else Offline mode
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (dataSnapshot.getValue() == null) {
                    ...
                    if (location != null) {
                        ...
                        //Sets GEO_HASHES in the Actiove list node
    ...
    public static void setUID(String mUID) {
        ...
        UID = mUID; //Unique user id generated by firebase
        Log.d(TAG, "User id is:" + UID);
        mMyProfileReference = mUserReference.child(UID);  // gives ur own node of profile
        mMyActiveReference = mActiveReference.child(UID); // ur own active list
        mMyBucketListReference = mBucketListRefernce.child(UID); // ur own bucket list
